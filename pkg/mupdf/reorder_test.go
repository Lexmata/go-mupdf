...
func TestReorderPages_Reverse(t *testing.T) {
	ctx, err := NewContext()
	if err != nil { t.Skipf("mupdf unavailable: %v", err) }
	defer ctx.Drop()
	...
	if err != nil { t.Fatalf("ReorderPages: %v", err) }

	count, err := PageCount(ctx, output)
	if err != nil { t.Fatalf("PageCount: %v", err) }
	if count != 5 { t.Errorf("expected 5 pages, got %d", count) }
}
...
func TestExtractBookmarks_AfterAdd(t *testing.T) {
	...
	if err != nil { t.Fatalf("ExtractBookmarks: %v", err) }
	if len(extracted) != 2 {
