...
func main() {
	...
	if _, err := os.Stat(mupdfDir); os.IsNotExist(err) {
		...
		if _, err := os.Stat(gitmodulesPath); os.IsNotExist(err) {
			...
			fmt.Fprintf(os.Stderr, "  git clone --recurse-submodules https://github.com/Lexmata/go-mupdf.git\n")
			fmt.Fprintf(os.Stderr, "\n")
			fmt.Fprintf(os.Stderr, "═══════════════════════════════════════════════════════════════\n")
			fmt.Fprintf(os.Stderr, "\n")
			os.Exit(1)
...
build:
	...
	if _, err := os.Stat(libMupdfThird); os.IsNotExist(err) {
		...
		fmt.Fprintf(os.Stderr, "  https://github.com/Lexmata/go-mupdf/issues\n")
		fmt.Fprintf(os.Stderr, "\n")
		fmt.Fprintf(os.Stderr, "═══════════════════════════════════════════════════════════════\n")
		fmt.Fprintf(os.Stderr, "\n")
		os.Exit(1)
